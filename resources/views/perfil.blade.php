...
<div style="max-width: 800px; margin: 0 auto;">
    ...
    <div style="background: white; border-radius: 16px; padding: 30px; margin-bottom: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
        <div style="display: flex; align-items: center; gap: 24px; margin-bottom: 24px; padding-bottom: 24px; border-bottom: 1px solid #e5e7eb;">
            <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #0B5E2E 0%, #0d7a3f 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 32px; font-weight: 600;">
                {{ strtoupper(substr($user->name, 0, 1)) }}
            </div>
            <div>
                <h2 style="font-size: 24px; font-weight: 600; color: #1f2937; margin-bottom: 4px;">{{ $user->name }}</h2>
                <span style="background: {{ $isAdmin ? '#C9A227' : '#0B5E2E' }}; color: white; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 500;">
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
            <div>
                ...
                <div style="font-size: 14px; color: #1f2937; display: flex; align-items: center; gap: 8px;">
                    <i class="fas fa-envelope" style="color: #0B5E2E;"></i>
                    {{ $user->email }}
                </div>
            </div>
            <div>
                ...
                <div style="font-size: 14px; color: #1f2937; display: flex; align-items: center; gap: 8px;">
                    <i class="fas fa-calendar" style="color: #0B5E2E;"></i>
    ...
    <div style="background: white; border-radius: 16px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
        <h3 style="font-size: 18px; font-weight: 600; color: #1f2937; margin-bottom: 20px; display: flex; align-items: center; gap: 8px;">
            <i class="fas fa-certificate" style="color: #C9A227;"></i>
            Mis Certificados
        </h3>
        ...
        @if($cursosCompletados->count() > 0)
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px;">
                @foreach($cursosCompletados as $progreso)
                    @if($progreso->curso)
                    <div style="background: linear-gradient(135deg, #0B5E2E 0%, #0d7a3f 100%); border-radius: 12px; padding: 20px; color: white; text-align: center;">
                        <i class="fas fa-award" style="font-size: 32px; margin-bottom: 12px; color: #C9A227;"></i>
                        ...
                        <a href="{{ route('certificado', $progreso->curso) }}" target="_blank" style="margin-top: 12px; background: #C9A227; color: #0B5E2E; border: none; padding: 8px 16px; border-radius: 6px; font-size: 12px; font-weight: 500; cursor: pointer; text-decoration: none; display: inline-block;">
